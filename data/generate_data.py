...
class MarketingDataGenerator:
    ...
    def generate_channel_metrics(self):
        ...
        dates = [
            self.start_date + timedelta(days=x) for x in range(self.date_range_days)
        ]
    ...
    def generate_complete_dataset(self):
        ...
        print(
            f"✓ Generated exposures (avg {exposure_df['n_touchpoints'].mean():.1f} touchpoints/user)"
        )
        ...
        print(
            f"✓ Generated conversions ({conversion_df['converted'].sum():,} conversions, {conversion_df['converted'].mean() * 100:.1f}% rate)"
        )
    ...
    def save_data(self, user_journey_df, channel_metrics_df):
        ...
        print(
            f"  Conversions: {user_journey_df['converted'].sum():,} ({user_journey_df['converted'].mean() * 100:.1f}%)"
        )
        print(f"  Total revenue: ${user_journey_df['revenue'].sum():,.0f}")
        print(
            f"  Avg revenue per conversion: ${user_journey_df[user_journey_df['converted'] == 1]['revenue'].mean():.2f}"
        )
        ...
        for segment in ["New", "Returning", "VIP"]:
            ...
            print(
                f"  {segment:10s}: {len(segment_data):6,} users, {conv_rate:5.2f}% conversion"
            )

        print("\nChannel Metrics:")
        print(
            f"  Date range: {channel_metrics_df['date'].min()} to {channel_metrics_df['date'].max()}"
        )
