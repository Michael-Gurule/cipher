...
def show_overview(journeys, channel_metrics):
    ...
    with col1:
        ...
        segment_stats = journeys.groupby('segment').agg({
    ...
    channel_spend = channel_metrics.groupby('channel')['spend'].sum().sort_values(ascending=False)
...
def show_channel_analysis(journeys, channel_metrics):
    ...
    st.info("⚠️ Note: 'Naive Lift' is correlational, not causal. It's biased by selection effects. See 'Causal Effects' page for true causal estimates.")
    ...
    fig.add_trace(
        go.Scatter(x=channel_time['date'], y=channel_time['spend'],
                  name='Spend', line=dict(color='blue')),
        secondary_y=False
    )
    
    fig.add_trace(
        go.Scatter(x=channel_time['date'], y=channel_time['clicks'],
                  name='Clicks', line=dict(color='green')),
        secondary_y=True
    )
...
def show_causal_effects(journeys):
    ...
    with st.spinner("Running Propensity Score Matching..."):
        ...
        covariates = ['age', 'intent_score', 'n_touchpoints',
                     'gender_M', 'segment_Returning', 'segment_VIP']
        ...
        if treatment.sum() < 100 or (1 - treatment).sum() < 100:
            st.warning(f"Insufficient data for {channel}. Need at least 100 exposed and 100 not exposed.")
            return
        ...
        st.write(f"**Confidence Interval:** [{results['ci_lower']*100:.2f}%, {results['ci_upper']*100:.2f}%]")
        ...
        if results['p_value'] < 0.05:
            if results['ate'] > 0:
                ...
                st.error(f"""
                ⚠️ **{channel} has a significant negative effect on conversion.**
            ...
            st.info(f"""
            ℹ️ **No significant effect detected for {channel}.**
...
def show_budget_optimizer(channel_metrics):
    ...
    st.write(f"**Total Budget:** ${total_allocation:,.0f} (Original: ${total_budget:,.0f})")
    
    if abs(total_allocation - total_budget) > 1000:
        st.warning(f"⚠️ Budget difference: ${abs(total_allocation - total_budget):,.0f}")
    ...
    st.info("💡 Note: This is a simplified model. Actual results depend on market conditions, competition, and seasonality.")
