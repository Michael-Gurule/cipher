...
class PropensityScoreMatching:
    ...
    def match(self, X, treatment, propensity_scores=None):
        ...
        for i, (treated_id, control_id, dist, valid) in enumerate(
            zip(treated_idx, control_idx[indices.flatten()], distances.flatten(), valid_matches)
        ):
            if valid:
        ...
        print(f"Matched {len(self.matched_pairs)} pairs ({len(self.matched_pairs)/len(treated_idx)*100:.1f}% of treated units)")
        print(f"Dropped {len(treated_idx) - len(self.matched_pairs)} treated units (outside caliper)")
    ...
    def check_balance(self, X, treatment, matched_pairs=None, threshold=0.1):
        ...
        for col_idx, col_name in enumerate(X.columns if hasattr(X, 'columns') else range(X.shape[1])):
            # Before matching
            treated_before = X[treatment == 1].iloc[:, col_idx] if hasattr(X, 'iloc') else X[treatment == 1, col_idx]
            control_before = X[treatment == 0].iloc[:, col_idx] if hasattr(X, 'iloc') else X[treatment == 0, col_idx]
            
            smd_before = self._calculate_smd(treated_before, control_before)
            
            # After matching
            treated_after = X.iloc[matched_pairs['treated_idx'], col_idx] if hasattr(X, 'iloc') else X[matched_pairs['treated_idx'], col_idx]
            control_after = X.iloc[matched_pairs['control_idx'], col_idx] if hasattr(X, 'iloc') else X[matched_pairs['control_idx'], col_idx]
        ...
        print(f"  Balanced covariates: {balance_df['balanced'].sum()}/{len(balance_df)}")
...
class DifferenceInDifferences:
    ...
    def test_parallel_trends(self, df, outcome_col, time_col, group_col, pre_periods):
        ...
        print(f"  Result: {'✓ Parallel trends supported' if result['parallel_trends_hold'] else '✗ Evidence against parallel trends'}")
...
def summarize_causal_results(results_dict, method_name):
    ...
    if 'ate' in results_dict:
        ...
        print(f"95% Confidence Interval: [{results_dict['ci_lower']:.4f}, {results_dict['ci_upper']:.4f}]")
    ...
    if 'treated_mean' in results_dict and 'control_mean' in results_dict:
        ...
        print(f"Difference: {results_dict['treated_mean'] - results_dict['control_mean']:.4f}")
